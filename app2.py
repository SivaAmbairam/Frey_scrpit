...
def run_script(script_name):
    ...
    def check_and_run_comparison():
        all_completed = all(
            status.startswith('Completed') for script, status in script_status.items() if script != 'Fisher Products.py')
        if all_completed:
            logging.debug("All scripts completed. Starting push_script.py.")
            # run_script('Data_Upload_to_Database.py')
            run_script('Compare_Products.py')
    ...
    def fisher_push(completed_script):
        if completed_script == 'Fisher Products.py':
            ...
            # run_script('Fisher_Data_Upload_to_Database.py')
            run_script('Compare_Fisher_Product.py')
...
UNWANTED_SCRIPTS = ['module_package.py', 'Data_Upload_to_Database.py', 'Fisher_Data_Upload_to_Database.py', 'Compare_Fisher_Product.py', 'Compare_Products.py']
